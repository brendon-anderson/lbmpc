...
d_min = -0.01;                	% uniform noise lower bound
d_max = 0.01;                 	% uniform noise upper bound
system = @(x,u,t) A_0*x + B*u ...       % nominal model
    + [0;g*T*sin(x(1))/L] ...           % nonlinearity
    + (d_min+(d_max-d_min).*rand(n,1)); % uniform noise

% control synthesis parameters
Q = diag([50;1]);             	% state deviation tuning matrix
R = 0.01*eye(m);               	% control effort tuning matrix
N = 5;                          % horizon length

% control synthesis constraints
lb_X = -0.7*ones(n,1);       	% state lower bound
ub_X = 0.7*ones(n,1);         	% state upper bound
lb_U = -7*ones(m,1);         	% input lower bound
ub_U = 7*ones(m,1);           	% input upper bound
...
stairs(t,X_lqr(1,:),'ro','markersize',3);
...
hold on;
box on;
stairs(t,U_lbmpc(:),'b*','markersize',3);
stairs(t,U_lqr(:),'r*','markersize',3);
stairs(t,U_mpc(:),'g*','markersize',3);
xlim([min(t),max(t)]);
xlabel('$t$','interpreter','latex');
ylabel('$u(t)$','interpreter','latex');
legend({'LBMPC','LQR','MPC'},...
    'interpreter','latex','location','southeast');
set(gca,'ticklabelinterpreter','latex');
