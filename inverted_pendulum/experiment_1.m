...
lb_X = -1*ones(n,1);            % state lower bound
ub_X = 1*ones(n,1);           	% state upper bound
lb_U = -10*ones(m,1);         	% input lower bound
ub_U = 10*ones(m,1);           	% input upper bound
...
hold on;
box on;
stairs(t,X_un(1,:),'bo','markersize',3);
stairs(t,X_un(2,:),'b*','markersize',3);
xlim([min(t),max(t)]);
